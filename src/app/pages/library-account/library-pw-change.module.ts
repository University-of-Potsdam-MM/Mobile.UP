...
export class LibraryPwChangePage implements OnInit {
  ...
  passwordsMustMatch: ValidatorFn = (control: FormGroup): ValidationErrors | null => {
    ...
    return newP && newVerify && newP.value === newVerify.value ? null : { 'passwordsDiffer': true };
  };
  ...
  async changePassword() {
    ...
    const headers = new HttpHeaders()
        .append('Authorization', 'Bearer ' + bibSession.token);
      
    this.http.post(this.endpoint + 'auth/change', body, { headers: headers }).subscribe(response => {
