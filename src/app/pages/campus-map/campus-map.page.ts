...
export class CampusMapPage extends AbstractPage implements AfterViewInit {
  ...
  constructor(
    ...
    private connectionService: ConnectionService // private http: HttpClient
  ) {
  ...
  setPosition(position: Geoposition) {
    ...
    if (
      (position && position.coords && position.coords.heading) ||
      this.latestHeading
    ) {
      // save current value
        ...
        if (!positionResponse.code) {
          ...
          this.logger.debug(
            ...
            `error getting position: ${
              positionResponse.message ? positionResponse.message : ""
            }`
          );
          disableCallback();
        }
  ...
  queryCampus(query: string | number) {
    if (ConfigService.config.campus) {
      return ConfigService.config.campus.find((campus: ICampus) => {
        return (
