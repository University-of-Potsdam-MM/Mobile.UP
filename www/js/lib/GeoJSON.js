			...
			if(geojsonProperties) {
				if (geojsonProperties.Name) {
					...
					if (geojsonProperties.id && hasSimilars(geojsonProperties.id)) {
						googleObj.info = new google.maps.InfoWindow({content: '<b>'+geojsonProperties.Name+desc+'<br><br><a href="#" onclick="event.preventDefault(); require([\'modules/sitemap\'], function(Sitemap) { new Sitemap().searchSimilarLocations(\''+geojsonProperties.id+'\'); });">Ähnliche Orte</a></b>'});
					} else {
